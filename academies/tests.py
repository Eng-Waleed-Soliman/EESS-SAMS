...
class ApplicationFlowsTests(TestCase):
    ...
    def test_reports_hub_contains_only_requested_management_reports(self):
        ...
        self.assertContains(response, 'إجمالي مشتروات أصناف الكافيتريا')
        self.assertContains(response, 'incomeCafeteriaPurchaseDetails')
        self.assertContains(response, 'صنف إحصائيات')
        self.assertContains(response, 'id="incomeDailyBookingDetails" class="cafeteria-detail-row d-none"')
        self.assertContains(response, 'id="incomeCafeteriaSalesDetails" class="cafeteria-detail-row d-none"')
        self.assertContains(response, 'إجمالي الحجز اليومي خلال الفترة')
        self.assertContains(response, 'إجمالي دخل الكافيتريا خلال الفترة')
    ...
    def test_revenue_share_subscriptions_sync_company_due_paid_and_supplied_amounts(self):
        ...
        self.assertEqual(subscriptions_page.context['academy_share_amount'], 1420)
